...
function init_province(province,city,country){
	ar=dsy.Items["0"];
	var ss=document.getElementById("province");
	for(j=0;j<ar.length;j++){
		if(ar[j]==province){
			ss.options[j+1].selected=true;
			change(1);//更改city的option值
			init_city("0_"+j,city,country);
		}
	}
}
function init_city(value,city,country){
	if(dsy.Exists(value)){
		ar=dsy.Items[value];
		...
		for(j=0;j<ar.length;j++){
			if(ar[j]==city){
				ss.options[j+1].selected=true;
				change(2);//更改country的值
				init_country(value+"_"+j,country);
			}
		}
	}
}

function init_country(value,country){
	if(dsy.Exists(value)){
		ar=dsy.Items[value];
		var ss=document.getElementById("country");
