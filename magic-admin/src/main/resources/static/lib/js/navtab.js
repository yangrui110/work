...
layui.define(['element'], function(exports){
   var  element = layui.element(),
		    ...
		    if(tabIndex === -1){
				...
				//监听关闭事件
				    ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
				    	element.tabDelete(ELEM.tabFilter, $(this).parent('li').index()).init('nav');
				    });
			    };
			    //切换到当前打开的选项卡
         ...
         * 关闭所有已经存在的tab选项
         * */
        LarryTab.prototype.tabCloseAll=function(){
        	ELEM.titleBox.children('li:not(:first)').remove();
        	ELEM.contentBox.children('.layui-tab-item:not(:first)').remove();
        }
        /**
         * 关闭除了当前的选项卡之外的所有选项卡
         * */
        LarryTab.prototype.tabCloseOther=function(){
        	ELEM.titleBox.children('li:not(:first,.layui-this)').remove();
        	ELEM.contentBox.children('.layui-tab-item:not(:first,.layui-show)').remove();	
        }
        /**
         * 刷新当前页面
         * */
        LarryTab.prototype.refreshCur=function(){
        	//获取当前页的信息
        	var href=ELEM.contentBox.children('.layui-show').children('.larry-iframe').attr("src");
        	var icon=ELEM.titleBox.children('.layui-this').children('li:first').attr('class');
        	var title=ELEM.titleBox.children('.layui-this').children('em').text();
        	var data = {
                    href: href,
                    icon: icon,
                    title: title
        	...
        	//关闭当前页
        	ELEM.titleBox.children('.layui-this').remove();
        	ELEM.contentBox.children('.layui-show').remove();	
        	//刷新当前页
        	var _this=this;
        	var tabIndex = _this.exists(data.title);
		    // 若不存在
